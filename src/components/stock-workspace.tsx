...
function Metric({ icon, label, value, detail, tone = "primary" }: { icon: React.ReactNode; label: string; value: string; detail: string; tone?: "primary" | "warning" | "danger" }) {
  ...
  return <article className={`grid min-w-0 grid-cols-[auto_minmax(0,1fr)] gap-3 overflow-hidden rounded-lg border p-4 ${style}`}><span className="mt-0.5">{icon}</span><div className="min-w-0"><p className="truncate text-xs text-muted-foreground">{label}</p><strong className="mobile-value text-xl text-foreground">{value}</strong><p className="truncate text-xs text-muted-foreground">{detail}</p></div></article>;
}
...
function PanelHeader({ title, description, action }: any) { return <header className="grid min-w-0 grid-cols-1 items-start gap-3 sm:grid-cols-[minmax(0,1fr)_auto]"><div className="min-w-0"><h2 className="text-lg font-semibold">{title}</h2><p className="text-sm text-muted-foreground">{description}</p></div>{action ? <div className="min-w-0 max-w-full justify-self-start sm:justify-self-end">{action}</div> : null}</header>; }
