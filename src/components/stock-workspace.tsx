...
export function StockWorkspace() {
  ...
  return <div className="space-y-4">
    ...
    <ModuleTabs value={tab} onValueChange={setTab} tabs={[
      ...
      { value: "items", label: "Itens", content: <ItemsPanel items={items} draft={itemDraft} setDraft={setItemDraft} editable={editable} loading={loading} save={(d) => itemMutation.mutate(d)} saving={itemMutation.isPending} remove={(id) => { if (confirm("Excluir este item? Os vínculos existentes podem ser afetados.")) removeItem.mutate(id); }} /> },
      { value: "warehouses", label: "Depósitos", content: <WarehousesPanel warehouses={warehouses} balances={availableBalances} draft={warehouseDraft} setDraft={setWarehouseDraft} editable={editable} loading={loading} save={(d) => warehouseMutation.mutate(d)} saving={warehouseMutation.isPending} remove={(id) => { if (confirm("Excluir este depósito?")) removeWarehouse.mutate(id); }} /> },
      { value: "movements", label: "Movimentações", content: <MovementsPanel items={items} warehouses={warehouses} movements={movements} receipts={receipts} transfers={transfers} pendingReceipts={pendingReceipts} movementDraft={movementDraft} setMovementDraft={setMovementDraft} transferDraft={transferDraft} setTransferDraft={setTransferDraft} editable={editable} loading={loading} openMovement={openMovement} openTransfer={openTransfer} saveMovement={(d) => movementMutation.mutate(d)} saveTransfer={(d) => transferMutation.mutate(d)} saving={movementMutation.isPending || transferMutation.isPending} remove={(id) => { if (confirm("Excluir esta movimentação manual?")) removeMovement.mutate(id); }} assignWarehouse={(data) => receiptWarehouseMutation.mutate(data)} assignItem={(data) => itemLinkMutation.mutate(data)} /> },
      { value: "inventory", label: "Inventário", content: <InventoryPanel items={items} warehouses={warehouses} counts={counts} draft={countDraft} setDraft={setCountDraft} editable={editable} loading={loading} save={(d) => countMutation.mutate(d)} saving={countMutation.isPending} apply={(id) => { if (confirm("Aplicar a diferença ao saldo do estoque?")) applyCountMutation.mutate(id); }} cancel={(id) => cancelCountMutation.mutate(id)} /> },
    ]} />
  </div>;
}
...
function MovementsPanel(props: any) {
  ...
  return <section className="space-y-6"><PanelHeader title="Movimentações" description="Histórico de entradas, saídas, transferências, ajustes e recebimentos de compras." action={editable ? <div className="flex flex-wrap gap-2"><Button variant="glow" onClick={() => openMovement("in")}><ArrowDownLeft className="h-4 w-4" /> Entrada</Button><Button variant="outline" onClick={() => openMovement("out")}><ArrowUpRight className="h-4 w-4" /> Saída</Button><Button variant="outline" onClick={openTransfer}><ArrowRightLeft className="h-4 w-4" /> Transferência</Button></div> : null} />
    ...
    {pendingReceipts.length ? <section className="space-y-3 rounded-lg border border-warning/30 bg-warning/5 p-4"><div><h3 className="font-semibold">Recebidos — aguardando armazenamento</h3><p className="text-xs text-muted-foreground">Vincule o depósito e o item para disponibilizar o saldo. A origem permanece no módulo Compras.</p></div>{pendingReceipts.map((r: ReceiptAllocation) => <article key={r.receipt_id} className="space-y-3 rounded-lg border border-border/60 bg-background/50 p-3"><div className="text-sm font-medium">{date(r.received_at)} • {r.supplier || "Fornecedor não informado"} {r.document ? `• ${r.document}` : ""}</div><Select value={r.warehouse_id ?? ""} disabled={!editable} onChange={(value) => assignWarehouse({ receiptId: r.receipt_id, warehouseId: value || null })} options={[["","Selecione o depósito"], ...warehouses.map((w: Warehouse) => [w.id,w.name])]} />{r.lines.map((line) => <div key={line.purchase_item_id} className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_240px] sm:items-center"><span className="text-sm">{line.description}</span><span className="text-xs text-muted-foreground">{qty(line.quantity)} {line.unit}</span><Select value={line.inventory_item_id ?? ""} disabled={!editable} onChange={(value) => assignItem({ purchaseItemId: line.purchase_item_id, inventoryItemId: value || null })} options={[["","Recebido — aguardando vínculo"], ...items.map((i: InventoryItem) => [i.id,i.name])]} /></div>)}</article>)}</section> : null}
